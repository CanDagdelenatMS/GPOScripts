...
(get-acl  "AD:\$($gpo.Path)").Access | where {$_.IsInherited -eq $false} `

        | Export-Csv -LiteralPath "$gpolocation\$currentdate\GPOPermissions\$($GPO.DisplayName).csv" #Filter for only deny permissions
...
Write-Host "Exporting All WMI Filters..." -ForegroundColor Yellow
...
if($WMIFilters)
    ...
    {$WMIFilters | Export-Csv "$gpolocation\$currentdate\WMI.csv"}
